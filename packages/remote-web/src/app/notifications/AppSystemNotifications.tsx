...
import { showSystemNotification } from "@remote/app/notifications/showSystemNotification";

/**
 * Mounts at app root in the remote/cloud UI (vibing.andersontechsolutions.com).
 *
 * Watches the user's notification stream and dispatches browser/OS toasts
 * for new groups. Mirrors the behavior of local-web's AppSystemNotifications
 * but without the Tauri code path (the cloud UI never runs in Tauri).
 *
 * Initial sync is silent: existing unseen notifications at app load do not
 * fire toasts — only items that arrive after init do.
 */
